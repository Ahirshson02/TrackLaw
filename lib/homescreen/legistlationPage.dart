...
import 'package:tracklaw/homescreen/chatInterface.dart';
import 'homescreen.dart';
import '/main.dart';
import 'package:tracklaw/api/firebaseAPI.dart';
import 'package:tracklaw/api/firebaseAPI.dart' as firebase;
import 'package:tracklaw/homescreen/chatInterface.dart' as chatInterface;

class LegistlationPage extends StatefulWidget {
  final Bill bill;
  final BillActions action;

  LegistlationPage({Key? key, required this.bill, required this.action})
      : super(key: key);
  @override
...
class _LegislationPageState extends State<LegistlationPage> {
  ...
  final apikey = 'AIzaSyAV_l2AufaBT4lBvl8IWt2-_K9JBMxFnic';
                          ...
                          children: [
                            ElevatedButton(
                              style: ElevatedButton.styleFrom(
                                backgroundColor:
                                    Color.fromARGB(255, 236, 189, 142),
                            ...
                            IconButton(
                                onPressed: () {
                                  Navigator.pop(
                                      context); //download and save as a pdf
                                },
  ...
  Future<void> getSummary(Bill bill) async {
    List<BillSummary> summaries = await congressAPI.getBillSummaries(
        bill.congress!, bill.billType!.toLowerCase(), bill.billNumber!);
    String errorMessage =
        "It looks like Congress has not yet created a summary for this bill. Check back later for updates";
