...
class FirestoreService {
  ...
  Future<List<Bill>> getFollowedBills(String userId) async {
    ...
    final followedBillIds = List<String>.from(userData['followedBillIds'] ?? []);
    ...
    for (int i = 0; i < followedBillIds.length; i += 10) {
      final end = (i + 10 < followedBillIds.length) 
          ? i + 10 
          : followedBillIds.length;
      
      final batch = followedBillIds.sublist(i, end);
      
      final querySnapshot = await _billsCollection
  ...
  Stream<List<ChatMessage>> streamMessagesForBill(String billId, String userId) {
  ...
  Future<void> recordUserBillInteraction(String userId, String billId, String type) async { // ------------------ delete?
